...
public class SceneNavigator : MonoBehaviour
{
    ...
    public string GetSceneCandidates()
    {
        ...
        payload.status = "ok";
        payload.message = $"Found {candidates.Count} candidate(s).";
    ...
    public string BuildVoxelObject(string targetId, string blocksJson = null)
    {
        ...
        if (!TryGetBuildPoseForTarget(candidate.Anchor, out var buildOrigin, out var buildRotation, out error))
        {
            payload.status = "error";
            payload.message = error;
            return JsonUtility.ToJson(payload);
        ...
        voxelBuildCounter++;
        var objectName = $"VoxelObject_{voxelBuildCounter}_{candidate.Info.name}";
        var capturedOffsets = blockOffsets;

        StartAnimatedMove(destination, () => SpawnVoxelObject(objectName, buildOrigin, buildRotation, capturedOffsets));

        payload.status = "ok";
        payload.message = $"Moving to {candidate.Info.name} and building {capturedOffsets.Count} block(s).";
        payload.label = candidate.Info.label;
        payload.name = candidate.Info.name;
